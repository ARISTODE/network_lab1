  ...
  def handle_peer(self, peer_socket, addr) -> None:
    ...
    try:
      ...
      while True:
        ...
        split = re.split(f'(?<={"}"})', msg)[:-1]

        #split incase multiple packets were recieved
        for j in split:
          command = json.loads(j)
          ...
          if cmd_type == 'register_file_chunk':
            ...
            if peer_id in self.peers:
              ...
              self.peers[peer_id]['files'][filename]['chunks'].append(chunk_id)
            ...
            file_info = {
                peer: {
                  'chunks': list(data['files'].get(cmd_filename, {}).get('chunks',{})),
